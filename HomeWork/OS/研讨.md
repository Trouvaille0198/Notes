...
习惯 / 一般操作：在 bash 界面实时输入命令，屏幕实时输出

遇到的情况：find 查找命令，耗时，占用屏幕不能干其他事情

问题：如果想让进程暂停或是转移到后台运行，从而腾出命令行终端去做别的事情，该怎么操作

了解 Linux 中实现进程后台运行、挂起的操作

又一个问题：多道进程执行时，该如何手动修改优先级

了解 Linux 中查看进程信息、修改优先级的操作

通过一个具体的例子，来领会 Linux 中的进程操作
...
>
> - `nohup` 还会将将输出重定向至运行路径的 `nohup.out` 上，不会在屏幕上输出了
