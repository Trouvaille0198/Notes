...
date modified: 24-04-10 22:13
banner: "https://markdown-1303167219.cos.ap-shanghai.myqcloud.com/Moby-share.png"
---
...
除此以外，`docker image ls` 还支持强大的**过滤器参数** `--filter`，或者简写 `-f`。
...
因此当我们使用上面命令删除镜像的时候，实际上是在**要求删除某个标签的镜像**。这就是我们看到的 `Untagged` 的信息。
...
可以使用 `docker container rm`（或 `docker rm`）来删除一个**处于终止状态**的容器。例如
